...
'@
    ...
    if (-not (Test-Path -LiteralPath $outputFullPath)) { throw 'The themed archive output was not created.' }
    $archiveListing = Invoke-Asar list $outputFullPath | Out-String
    $archiveCssPath = '\dist\assets\' + $cssName
    if (-not $archiveListing.Contains($archiveCssPath)) { throw 'Archive verification failed: the main stylesheet is missing.' }
